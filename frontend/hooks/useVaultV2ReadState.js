import { engineV2Abi, erc20Abi, farmAdapterAbi, managedAdapterAbi, oracleAbi, pegArbAbi, policyV2Abi, vaultV2Abi } from "@/lib/abi";
...
export function useVaultV2ReadState() {
  ...
  const [lpStakingInfo, setLpStakingInfo] = useState({ staked: 0n, unstaked: 0n, pending: 0n });
  ...
  const refresh = useCallback(async ({
    ...
    try {
      ...
      const vault = new ethersLib.Contract(ethersLib.getAddress(vaultAddress.trim()), vaultV2Abi, runner);
      const engine = new ethersLib.Contract(ethersLib.getAddress(engineAddress.trim()), engineV2Abi, runner);
      ...
      const [
        ...
        rawRiskScore, rawTimeUntilNext, rawCycleCount, isLocked,
      ...
      if (circuitBreakerAddress) {
        const breakerPreview = await engine.previewBreaker().catch(() => null);
        if (breakerPreview) {
          setBreakerStatus({
            paused: breakerPreview.status?.paused ?? breakerPreview.paused ?? false,
            signalA: breakerPreview.status?.signalA ?? breakerPreview.signalA ?? false,
            signalB: breakerPreview.status?.signalB ?? breakerPreview.signalB ?? false,
            signalC: breakerPreview.status?.signalC ?? breakerPreview.signalC ?? false,
            lastTripTimestamp: breakerPreview.status?.lastTripTimestamp ?? breakerPreview.lastTripTimestamp ?? 0n,
            recoveryTimestamp: breakerPreview.status?.recoveryTimestamp ?? breakerPreview.recoveryTimestamp ?? 0n,
          });
        }
      }
      ...
      const healthScoreResult = await engine.vaultHealthScore().catch(() => null);
      if (healthScoreResult) {
        ...
        try { setVaultHealthLabel(ethersLib.decodeBytes32String(rawLabel)); } catch { setVaultHealthLabel(null); }
      }

      // Peg arbitrage preview
      if (pegArbExecutorAddress) {
        const pegArbContract = new ethersLib.Contract(ethersLib.getAddress(pegArbExecutorAddress.trim()), pegArbAbi, runner);
        const arbPreviewData = await pegArbContract.previewArb().catch(() => null);
        if (arbPreviewData) {
      ...
      let rawAsterManaged = null, rawSecondaryManaged = null, rawLpManaged = null;

      if (policyAddress && oracleAddress) {
        const policy = new ethersLib.Contract(policyAddress, policyV2Abi, runner);
      ...
      if (rawLpAdapterAddress) {
        const lpAdapterContract = new ethersLib.Contract(rawLpAdapterAddress, managedAdapterAbi, runner);
        rawLpManaged = await lpAdapterContract.managedAssets().catch(() => null);
        setLpAdapterAddress(rawLpAdapterAddress);

        // Query LP staking info + gas harvest params if farm adapter exists
        const lpAdapterWithFarm = new ethersLib.Contract(rawLpAdapterAddress, farmAdapterAbi, runner);
        const [stakingResult, rawHarvestGasEstimate, rawHarvestGasMultiplier] = await Promise.all([
          lpAdapterWithFarm.stakingInfo().catch(() => ({ staked: 0n, unstaked: 0n, pending: 0n })),
          lpAdapterWithFarm.harvestGasEstimate().catch(() => null),
          lpAdapterWithFarm.harvestGasMultiplier().catch(() => null),
        ]);
        setHarvestGasEstimate(rawHarvestGasEstimate != null ? toSafeNumber(rawHarvestGasEstimate) : null);
        setHarvestGasMultiplier(rawHarvestGasMultiplier != null ? toSafeNumber(rawHarvestGasMultiplier) : null);
        if (stakingResult && Array.isArray(stakingResult)) {
          setLpStakingInfo({ staked: stakingResult[0] ?? 0n, unstaked: stakingResult[1] ?? 0n, pending: stakingResult[2] ?? 0n });
        } else if (stakingResult && typeof stakingResult === 'object') {
          setLpStakingInfo({ staked: stakingResult.staked ?? 0n, unstaked: stakingResult.unstaked ?? 0n, pending: stakingResult.pending ?? 0n });
        }
      }
  ...
  return {
    ...
    totalAssetsRaw, asterManagedAssets, secondaryManagedAssets, lpManagedAssets, lpAdapterAddress, lpStakingInfo,
    bufferStatus, pendingWithdrawals, userTokenBalance,
    idleBufferBps, breakerStatus, sharpeMetrics, auctionMetrics, arbPreview,
    vaultHealthScore, vaultHealthLabel,
    harvestGasEstimate, harvestGasMultiplier,
    refresh,
    toSafeNumber,
  };
}
