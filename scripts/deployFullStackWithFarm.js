 ...
 *   Rail 3: StableSwapLPYieldAdapterWithFarm  (PCS StableSwap LP + MasterChef staking + CAKE harvest)
 *
 * Required env vars (see .env.example):
 *   V2_ASSET_ADDRESS, V2_MINTER_ADDRESS, V2_STABLESWAP_POOL,
 *   V2_CHAINLINK_FEED, V2_USDF_ADDRESS, V2_USDF_MINTING_ADDRESS
 *
 * Farm-specific env vars (REQUIRED):
 *   V2_PANCAKE_ROUTER      — PancakeSwap router address (V2: 0x10ED43C718714eb63d5aA57B78B54704E256024E)
 *   V2_CAKE_TOKEN          — CAKE token address (0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82)
 *   V2_MASTERCHEF          — MasterChef V3 address (0x556B9306565093C855AEA9AE92A594704c2Cd59e)
 *   V2_LP_POOL_ID          — Pool ID in MasterChef for USDF/USDT LP (TBD)
 *
 * LP-specific env vars (defaults provided):
 *   V2_LP_TOKEN_ADDRESS    — LP token address (defaults to pool address)
 *
...
async function main() {
  ...
  const cakeToken = requiredEnv("V2_CAKE_TOKEN");
  const masterChef = requiredEnv("V2_MASTERCHEF");
  const lpPoolId = requiredEnv("V2_LP_POOL_ID");
  ...
  console.log("=== Deploying ProofVault V2 + Farm Integration stack ===");
  ...
  console.log("\n[5/10] Deploying AsterEarnAdapterWithSwap (USDT→USDF swap)...");
  ...
  // ── [7] StableSwapLPYieldAdapterWithFarm (LP rail + farm) ───────────────────
  console.log("\n[7/10] Deploying StableSwapLPYieldAdapterWithFarm (LP + MasterChef + CAKE harvest)...");
  ...
  const lpAdapter = await StableSwapLPYieldAdapterWithFarm.deploy(
    ...
    lpPoolId,        // NEW: Pool ID in MasterChef
    deployer.address
  );
  await lpAdapter.waitForDeployment();
  console.log("StableSwapLPYieldAdapterWithFarm:", await lpAdapter.getAddress());
  ...
  // ── [9] StrategyEngine ─────────────────────────────────────────────────────
  console.log("\n[9/10] Deploying StrategyEngine (with harvestRewards integration)...");
  ...
  // ── Deployment Summary ───────────────────────────────────────────────────────
  console.log("\n=== DEPLOYMENT SUMMARY (FARM INTEGRATION) ===");
  console.log("Network:", hre.network.name);
  console.log("Deployer:", deployer.address);
  ...
  console.log("  CAKE Token:              ", cakeToken);
  ...
  console.log("  CircuitBreaker:          ", await breaker.getAddress());
  console.log("  SharpeTracker:           ", await sharpeTracker.getAddress());
  console.log("  AsterAdapter (WithSwap): ", await asterAdapter.getAddress());
  console.log("  SecondaryAdapter:        ", await secondaryAdapter.getAddress());
  console.log("  LpAdapter (WithFarm):    ", await lpAdapter.getAddress());
  console.log("  PegArbExecutor:          ", await pegArb.getAddress());
  ...
  console.log("\nDeployment complete.");
  console.log("\n🎯 ROBOT ROUTE ENABLED:");
  console.log("  1. User deposits USDT → Vault");
  console.log("  2. Engine swaps USDT → USDF (PancakeSwap)");
  console.log("  3. Engine deposits USDF → AsterDEX Earn → receives asUSDF");
  console.log("  4. Engine adds liquidity to USDF/USDT pool → receives LP tokens");
  console.log("  5. Engine stakes LP tokens in MasterChef → earns CAKE");
  console.log("  6. executeCycle() harvests CAKE → swaps to USDT → redeploys");
