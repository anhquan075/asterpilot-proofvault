...
describe("ProofVault V2 — Farm Integration Tests", function () {
  ...
  async function deployFarmIntegrationFixture() {
    ...
    const vault = await ProofVault.deploy(
      usdt.target, "ProofVault V2 Farm", "pvFARM", deployer.address, 500
    );
    ...
    return {
      ...
      usdt, usdf, cake, vault, engine, policy, oracle, breaker, sharpeTracker,
      asterAdapter, secondaryAdapter, lpAdapter, asterMinter,
      chainlinkFeed, stableSwapPool, pancakeRouter, masterChef, poolId
    };
  }

  describe("Farm Adapter Deployment", function () {
    it("Should deploy with correct farm parameters", async function () {
      const { lpAdapter, cake, masterChef, poolId } = await loadFixture(deployFarmIntegrationFixture);
    ...
    it("Should have default harvest thresholds", async function () {
      ...
      expect(await lpAdapter.minCakeHarvestAmount()).to.equal(ethers.parseUnits("1", 18));
  ...
  describe("LP Staking Integration", function () {
    it("Should stake LP tokens after adding liquidity", async function () {
      const { vault, engine, user, usdt, lpAdapter, masterChef, poolId, stableSwapPool } = await loadFixture(deployFarmIntegrationFixture);
    ...
    it("Should display staking info correctly", async function () {
      const { vault, engine, user, usdt, lpAdapter } = await loadFixture(deployFarmIntegrationFixture);
  ...
  describe("CAKE Harvest", function () {
    it("Should harvest CAKE rewards after time passes", async function () {
      const { vault, engine, user, usdt, lpAdapter, masterChef, cake } = await loadFixture(deployFarmIntegrationFixture);
    ...
    it("Should swap CAKE to USDT via PancakeSwap", async function () {
      const { vault, engine, user, lpAdapter, masterChef, cake, usdt } = await loadFixture(deployFarmIntegrationFixture);
    ...
    it("Should skip harvest if below minimum threshold", async function () {
      const { vault, engine, user, lpAdapter, masterChef } = await loadFixture(deployFarmIntegrationFixture);
      ...
      await impersonateAccount(vault.target);
      const vaultSigner = await ethers.getSigner(vault.target);
      const usdtReturned = await lpAdapter.connect(vaultSigner).harvestRewards.staticCall();
      expect(usdtReturned).to.equal(0);
    });

    it("Should return harvested USDT to vault", async function () {
      const { vault, engine, user, usdt, lpAdapter, masterChef } = await loadFixture(deployFarmIntegrationFixture);
      ...
      await setBalance(vault.target, ethers.parseEther("1"));
      const vaultSigner = await ethers.getSigner(vault.target);
      const usdtReturnedStatic = await lpAdapter.connect(vaultSigner).harvestRewards.staticCall();
      const tx = await lpAdapter.connect(vaultSigner).harvestRewards();
      const vaultBalanceAfter = await usdt.balanceOf(vault.target);
      expect(vaultBalanceAfter).to.be.gt(vaultBalanceBefore);
    });
  ...
  describe("executeCycle Integration", function () {
    it("Should call harvestRewards during executeCycle", async function () {
      const { vault, user, engine, lpAdapter, masterChef } = await loadFixture(deployFarmIntegrationFixture);
    ...
    it("Should redeploy harvested USDT in next rebalance", async function () {
      const { vault, user, engine, usdt, asterAdapter, lpAdapter, masterChef } = await loadFixture(deployFarmIntegrationFixture);
    ...
    it("Should handle non-farm adapter gracefully", async function () {
      const { vault, engine, user, usdt, deployer } = await loadFixture(deployFarmIntegrationFixture);
      ...
      const vault2 = await ProofVault.deploy(
        usdt.target, "Test", "TEST", deployer.address, 500
      );

      const StrategyEngine = await ethers.getContractFactory("StrategyEngine");
      const engine2 = await StrategyEngine.deploy(
        vault2.target, policy2.target, oracle2.target, breaker2.target, sharpe2.target, 100000000n
      );
      ...
      await vault2.connect(user).deposit(ethers.parseUnits("10000", 18), user.address);
  ...
  describe("USDT to USDF Swap", function () {
    it("Should swap USDT to USDF before depositing to AsterDEX", async function () {
      const { vault, engine, user, usdt, usdf, asterAdapter, pancakeRouter } = await loadFixture(deployFarmIntegrationFixture);
