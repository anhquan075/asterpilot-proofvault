...
async function main() {
  ...
  const usdf = await (await MockERC20.deploy("Aster USD", "USDF")).waitForDeployment();
  const cake = await (await MockERC20.deploy("PancakeSwap Token", "CAKE")).waitForDeployment();
  const wbnb = await (await MockERC20.deploy("Wrapped BNB", "WBNB")).waitForDeployment();
  ...
  const masterChef = await (await MockMasterChef.deploy(await cake.getAddress())).waitForDeployment();
  ...
  await (await masterChef.addPool(await pool.getAddress(), { gasLimit: GAS_LIMIT })).wait();
  ...
  const chainlinkFeed = await (await MockChainlink.deploy(8, hre.ethers.parseUnits("1", 8))).waitForDeployment();
  console.log("ChainlinkFeed (Mock):", await chainlinkFeed.getAddress());

  const MockUSDFMinting = await hre.ethers.getContractFactory("MockUSDFMinting");
  const usdfMinting = await (await MockUSDFMinting.deploy(await usdf.getAddress(), await usdt.getAddress())).waitForDeployment();
  ...
  const oracle = await (await MockPriceOracle.deploy(initialPrice, deployer.address)).waitForDeployment();
  await (await oracle.lock({ gasLimit: GAS_LIMIT })).wait();
  ...
  const sharpeTracker = await (await SharpeTracker.deploy(sharpeWindowSize)).waitForDeployment();
  console.log("[4] SharpeTracker:", await sharpeTracker.getAddress());

  const AsterEarnAdapterWithSwap = await hre.ethers.getContractFactory("AsterEarnAdapterWithSwap");
  const asterAdapter = await (await AsterEarnAdapterWithSwap.deploy(
    await usdt.getAddress(),
    await usdf.getAddress(),
  ...
  await (await usdf.mint(await pool.getAddress(), SWAP_SEED, { gasLimit: GAS_LIMIT })).wait();
  ...
  const StableSwapLPYieldAdapterWithFarm = await hre.ethers.getContractFactory("StableSwapLPYieldAdapterWithFarm");
  const lpAdapter = await (await StableSwapLPYieldAdapterWithFarm.deploy(
    await usdt.getAddress(),
    await pool.getAddress(),
  ...
  await (await sharpeTracker.setEngine(engineAddr, { gasLimit: GAS_LIMIT })).wait();
  console.log("[9] StrategyEngine:", engineAddr);

  const PegArbExecutor = await hre.ethers.getContractFactory("PegArbExecutor");
  const pegArb = await (await PegArbExecutor.deploy(
    vaultAddr,
    await usdt.getAddress(),
    await usdf.getAddress(),
  ...
  const ExecutionAuction = await hre.ethers.getContractFactory("ExecutionAuction");
  const executionAuction = await (await ExecutionAuction.deploy(
    engineAddr,
    vaultAddr,
    await usdt.getAddress(),
  ...
  // ── [PHASE 3] Wire ──────────────────────────────────────────────────────────
  console.log("\n--- Phase 3: Wiring ---");
  await (await vault.setEngine(engineAddr, { gasLimit: GAS_LIMIT })).wait();
  ...
  await (await vault.setPegArbExecutor(await pegArb.getAddress(), { gasLimit: GAS_LIMIT })).wait();
  await (await asterAdapter.setVault(vaultAddr, { gasLimit: GAS_LIMIT })).wait();
  await (await secondaryAdapter.setVault(vaultAddr, { gasLimit: GAS_LIMIT })).wait();
  await (await lpAdapter.setVault(vaultAddr, { gasLimit: GAS_LIMIT })).wait();
  console.log("All adapters wired.");

  // ── [PHASE 4] Lock ──────────────────────────────────────────────────────────
  console.log("\n--- Phase 4: Locking ---");
  await (await asterAdapter.lockConfiguration({ gasLimit: GAS_LIMIT })).wait();
  await (await secondaryAdapter.lockConfiguration({ gasLimit: GAS_LIMIT })).wait();
  await (await lpAdapter.lockConfiguration({ gasLimit: GAS_LIMIT })).wait();
  await (await vault.lockConfiguration({ gasLimit: GAS_LIMIT })).wait();
